...
export default function HomePage() {
  ...
  const byCategory = (en) => articles.filter((a) => a.category?.en === en || a.category === en);
      ...
      <div className="shell">
        ...
        <section className="hero">
          <div className="hero-content">
            ...
            <h1 className="hero-headline kn">
              <Link href={`/article/${lead.slug}`}>{t(lead.title, lang)}</Link>
            </h1>
            <p className="hero-dek kn">{t(lead.dek, lang)}</p>
        ...
        <section>
          <div className="section-head">
            <div className="section-title">
              ...
              <h2 className="kn">{lang === 'kn' ? 'ಇತ್ತೀಚಿನ ಸುದ್ದಿ' : 'Latest News'}</h2>
            </div>
          </div>
          <div className="secondary-grid">
            {secondary.map((article) => (
              <article key={article.slug}>
                <span className="tag mono kn">{t(article.tag, lang)}</span>
        ...
        <section className="feature-strip">
          <div className="feature">
            <span className="mini">{lang === 'kn' ? 'ನೆಲಮಟ್ಟದ ವರದಿ' : 'GROUND REPORT'}</span>
            <h3 className="kn">{lang === 'kn' ? 'ಜನರ ಧ್ವನಿ, ನೇರ ವರದಿ' : "The People's Voice, Direct Reporting"}</h3>
            <p className="kn">
              {lang === 'kn'
          ...
          <div className="feature">
            <span className="mini">{lang === 'kn' ? 'ವಿವರಣೆ' : 'EXPLAINER'}</span>
            <h3 className="kn">{lang === 'kn' ? 'ಸುದ್ದಿಯ ಹಿಂದೆ ಏನಿದೆ?' : "What's Behind the News?"}</h3>
            <p className="kn">
              {lang === 'kn'
          ...
          <div className="feature">
            <span className="mini">{lang === 'kn' ? 'ಅಭಿಪ್ರಾಯ' : 'OPINION'}</span>
            <h3 className="kn">{lang === 'kn' ? 'ಚರ್ಚೆಗೆ ವೇದಿಕೆ' : 'A Platform for Debate'}</h3>
            <p className="kn">
              {lang === 'kn'
        ...
        {categoriesWithContent.map((cat, i) => (
          <section className="category-block" id={cat.slug} key={cat.slug} style={i > 0 ? { borderTop: '1px solid var(--line)' } : undefined}>
